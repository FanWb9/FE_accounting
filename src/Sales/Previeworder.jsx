...
export default function Previeworder() {
  ...
  const data = state?.transactionData;
    const handleDownloadExcel = () => {
      ...
      const wsData = [
        ...
        [],
        ["BUKTI PESANAN PENJUALAN"],
        [`Tanggal Transaksi:`, data.tran_date],
  ...
  useEffect(() => {
    ...
    doc.setFontSize(14);
    doc.text("PT SAMPOERNA", marginLeft, y);
    ...
    doc.setFont("helvetica", "bold");
    doc.text("PESANAN PENJUALAN", 105, (y += 14), { align: "center" });

    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "normal");
    y += 10;
    doc.text(`Tanggal Transaksi : ${data.tran_date}`, marginLeft, (y += 8));
    doc.text(`No Pesanan        : ${data.reference}`, marginLeft, (y += 6));

    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(33, 33, 33);
    doc.text("Detail Barang", marginLeft, (y += 12));

    autoTable(doc, {
      startY: y + 3,
      head: [['No', 'Barang', 'Jumlah']],
      body: data.stock_id.map((akun, idx) => [
        idx + 1,
        akun,
