...
export default function SalesInvoice() {
  ...
  useEffect(() => {
    ...
    if (id && barangOptions.length > 0 && lokasiOptions.length > 0 && !dataFetched) {
      ...
      axios.get(`${API_URL}/sales/transaction-detail/${id}`)
        .then((response) => {
 ...
 const handleSubmit = async () => {
  ...
  const transactionData = {
    ...
    marketing: selectedSalesman.value,
    gudang: selected.value,
    reference: textInput,
    tran_date: date,
    ov_amount: total.toString(),
  ...
  try {
    let url = `${API_URL}/sales/Transaction`;
    let method = "post";
    
    // If in edit mode, use PUT endpoint
    if (isEditMode && id) {
      url = `${API_URL}/sales/Transaction/${id}`;
      method = "put";
    }
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-white p-6 text-black">
          <h1 className="text-2xl font-bold text-center">
            {isEditMode ? "Edit Transaksi Penjualan" : "Transaksi Penjualan"}
          </h1>
        </div>
