...
export default function SalesOrder() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${API_URL}/sales/pelanggan`),
      axios.get(`${API_URL}/sales/salesman`),
      axios.get(`${API_URL}/sales/name`),
      axios.get(`${API_URL}/sales/money`),
      axios.get(`${API_URL}/sales/barang`)
    ]).then(([pelangganRes, salesmanRes, lokasiRes, moneyRes, barangRes]) => {

        // Process data pelanggan
      setpelangganOptions(pelangganRes.data.map((item) => ({
        value: item.debtor_no,
        label: `${item.debtor_no} | ${item.name}`,
      })));

        // Process data salesman
      setsalesmanOptions(salesmanRes.data.map((item) => ({
  ...
  useEffect(() => {
    ...
    if (id && barangOptions.length > 0 && lokasiOptions.length > 0 && !dataFetched) {
      ...
      axios.get(`${API_URL}/sales/transactionorder-detail/${id}`)
        .then((response) => {
          ...
          if (response.data && response.data.transaction) {
            ...
            setTextInput(transaction.reference || "");
            setDate(transaction.tran_date ? transaction.tran_date.split("T")[0] : new Date().toISOString().split("T")[0]);
            ...
            const selectedSalesman = salesmanOptions.find(salesman => salesman.value === transaction.marketing);
            if (selectedSalesman) {
                console.log("Setting salesman:", selectedSalesman);
                setSelectedSalesman(selectedSalesman);
            }

            // Set selected customer
            const selectedCustomer = pelangganOptions.find(pelanggan => pelanggan.value === transaction.debtor_no);
            if (selectedCustomer) {
                console.log("Setting pelanggan:", selectedCustomer);
                setSelectedCustomer(selectedCustomer);
            }
            
            // Set selected gudang
            const selectedGudang = lokasiOptions.find(location => location.value === transaction.gudang);
            if (selectedGudang) {
            ...
            const selectedCurrency = moneyOptions.find(curr => curr.value === transaction.Currency);
            if (selectedCurrency) {
  ...
  const fetchGLTransactions = (transNo) => {
    ...
    setIsLoading(true);
    axios.get(`${API_URL}/sales/gl-transactions/${transNo}`)
      .then(response => {
  ...
  const handleAdd = () => {
    ...
    if (quantity.includes(',')) {
      ...
      cleanQuantity = parseFloat(quantity.replace(/\./g, "").replace(",", "."));
    } else {
      // Just remove dots for integers
      cleanQuantity = parseFloat(parseNumber(quantity));
    }

    let cleanUnitPrice;
    if (unitprice.includes(',')) {
      // Convert from "19.891,09" format to floating point number
      cleanUnitPrice = parseFloat(unitprice.replace(/\./g, "").replace(",", "."));
    } else {
      // Just remove dots for integers
      cleanUnitPrice = parseFloat(parseNumber(unitprice));
    }
    ...
    if (isEditing && editingId !== null) {
      setData(prev => prev.map(item => 
        item.id === editingId 
          ? { ...item, akunbarang: selectedChart.label, keterangan, quantity: cleanQuantity, unitprice: cleanUnitPrice, jumlah: cleanJumlah, selectedChart }
          : item
      ));
 ...
 const handleSubmit = async () => {
  // Validation
  if (!selectedCustomer ||!selectedSalesman ||!selected || !selectedMoney || !textInput || !date) {
  ...
  const quantityPerBarang = data.map(item => item.quantity.toString());
  const unitpricePerBarang = data.map(item => item.unitprice.toString());
  ...
  const transactionData = {
    ...
    debtor_no: selectedCustomer.value,
    salesman: selectedSalesman.value,
    from_stk_loc: selected.value,
    reference: textInput,
    ord_date: date,
    total: total.toString(),
    Currency: selectedMoney.value,
    stk_code: acc,
    quantity: quantityPerBarang,
    unit_price: unitpricePerBarang,
  ...
  try {
    let url = `${API_URL}/sales/TransactionOrder`;
    let method = "post";
    
    // If in edit mode, use PUT endpoint
    if (isEditMode && id) {
      url = `${API_URL}/sales/TransactionOrder/${id}`;
      method = "put";
    }
    ...
    setTimeout(() => {
      setIsSaving(false);
      navigate("/previewinvoice", { state: { transactionData } });
    }, 1500);
  } catch (error) {
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-white p-6 text-black">
          <h1 className="text-2xl font-bold text-center">
            {isEditMode ? "Edit Transaksi Pesanan Penjualan" : "Transaksi Pesanan Penjualan"}
          </h1>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div>
              <label className="block text-gray-700 font-semibold mb-2">Pilih Pelanggan</label>
              <Select
            ...
            <div>
              <label className="block text-gray-700 font-semibold mb-2">Pilih Salesman</label>
              <Select
                ...
                <table className="w-full text-sm divide-y divide-gray-200">
                  <thead className="bg-gray-100 sticky top-0">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right font-medium text-gray-700">Kwantitas</th>
                      ...
                      filteredData.map((item, i) => (
                        <tr
                          ...
                          onClick={() => {
                            ...
                            setQuantity(item.quantity.toLocaleString('id-ID').replace(/,/g, '.'));
                            setUnitPrice(item.unitprice.toLocaleString('id-ID').replace(/,/g, '.'));
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (hasDecimal) {
                      ...
                      var harga=document.getElementById("unitprice-input")?.value.replace(/\D/g, "") * inputValue;
                      ...
                      if (rawValue === "" || /^0+$/.test(rawValue)) {
                        ...
                        var harga=document.getElementById("unitprice-input")?.value.replace(/\D/g, "") * rawValue;
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      if (quantity.includes(',')) {
                        ...
                        numericValue = parseFloat(quantity.replace(/\./g, "").replace(",", "."));
                      } else {
                        numericValue = parseFloat(quantity.replace(/\./g, ""));
                      }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (hasDecimal) {
                      ...
                      var jumlah=document.getElementById("quantity-input")?.value.replace(/\D/g, "") * inputValue;
                      setJumlah(jumlah);
                      ...
                      if (rawValue === "" || /^0+$/.test(rawValue)) {
                        ...
                        var jumlah=document.getElementById("quantity-input")?.value.replace(/\D/g, "") * rawValue;
                        setJumlah(jumlah);
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      if (unitprice.includes(',')) {
                        // Convert from "15.000,50" format to numeric value
                        numericValue = parseFloat(unitprice.replace(/\./g, "").replace(",", "."));
                      } else {
                        numericValue = parseFloat(unitprice.replace(/\./g, ""));
                      }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (hasDecimal) {
                      ...
                      var jumlah=document.getElementById("quantity-input")?.value.replace(/\D/g, "") * document.getElementById("unitprice-input")?.value.replace(/\D/g, "");
                      setJumlah(jumlah);
