...
export default function StokMasuk() {
  ...
  useEffect(() => {
    ...
    if (id && barangOptions.length > 0 && lokasiOptions.length > 0 && !dataFetched) {
      ...
      axios.get(`${API_URL}/produk/transactionmasuk-detail/${id}`)
        .then((response) => {
  ...
  const fetchGLTransactions = (transNo) => {
    ...
    setIsLoading(true);
    axios.get(`${API_URL}/produk/produkmasuk-transactions/${transNo}`)
      .then(response => {
  ...
  const processTransactionDetails = (details) => {
    ...
    // Filter out masuk transactions if needed
    const detailsData = details
      .filter(detail => {
        // Skip entries with negative amounts (the masuk entry)
 ...
 const handleSubmit = async () => {
  ...
  try {
    let url = `${API_URL}/produk/TransactionMasuk`;
    let method = "post";
    
    // If in edit mode, use PUT endpoint
    if (isEditMode && id) {
      url = `${API_URL}/produk/TransactionMasuk/${id}`;
      method = "put";
    }
    ...
    setTimeout(() => {
      setIsSaving(false);
      navigate("/Previewin", { state: { transactionData } });
    }, 1500);
  } catch (error) {
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-white p-6 text-black">
          <h1 className="text-2xl font-bold text-center">
            {isEditMode ? "Edit Transaksi Stok Masuk" : "Stok Masuk"}
          </h1>
        </div>
